...
这个结构很像 Git 的提交历史：每个对话条目通过 `parentId` 指向父条目，Lane 则像指向当前叶节点的分支；创建新分支只需增加一个指针，不必复制已有历史。
...
## Agent Runtime 正在走向分布式系统

如何安全地删除数据，是 Agent Runtime 进入生产后需要解决的问题之一。Harness V2 主要采用只追加设计：对话树中的条目不删除，操作记录也会持续累积。上下文压缩只能缩短模型看到的内容，不能阻止底层数据一直增长。

对于运行几个月甚至更久的会话，系统必须决定哪些数据可以删除、何时删除。已经完成的操作记录可能只剩审计价值，但用量记录、审计日志和对话内容各有不同的保留要求。合规删除、审计追溯、分支重建和存储压缩也会彼此制约。把数据分成对话事实和执行记录，只解决了运行时如何使用它们的问题，长期保存仍需要单独的数据治理设计。

这个问题也说明了 Agent Runtime 正在发生的变化。当 Agent 从本机短任务走向长时运行、Sub-agent 协作和托管服务，Harness 需要承担持久化、调度、恢复、并发控制、成本核算和数据治理等职责。此时它已经越来越接近工作流引擎和分布式运行时。

模型带来了新的不确定性，却没有让传统工程问题消失。预写日志、幂等性、单写者、租约、栅栏令牌、事件溯源和账目对账，都会在 Agent 系统中发挥作用。这些机制还要进一步处理模型生成的不确定性、工具产生的外部副作用，以及无法精确重放时的语义恢复。

Agent 可以从一个 `while` 循环开始。要让它长期、并行、托管地运行下去，数据库、分布式系统、工作流引擎和数据治理积累的方法都会变得越来越重要。
